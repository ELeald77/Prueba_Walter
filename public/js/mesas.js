...
$(function() {
  ...
  // Helpers UI
  function formatear(valor){return `Q${Number(valor||0).toLocaleString('es-CO')}`}
      ...
      productos.forEach(p => {
        const item = $(`
          <a href="#" class="list-group-item list-group-item-action">
            ...
            <div class="small text-muted">KG: Q${p.precio_kg} | UND: Q${p.precio_unidad} | LB: Q${p.precio_libra}</div>
          </a>`);
        item.on('click', e => {
          e.preventDefault();
  ...
  function buildPedidoResumenHtml(){
    ...
    const rows = (items||[]).map(it => {
      ...
      return `<tr><td>${nombre}</td><td class="text-end">${cantidad}</td><td class="text-end">Q${subtotal.toLocaleString('es-CO')}</td></tr>`;
    }).join('');
    return `
      <div class="border rounded p-2 mt-2" id="contenedorResumen" style="display:none;max-height:220px;overflow:auto;">
        <table class="table table-sm mb-2">
          ...
          <tbody>${rows}</tbody>
          <tfoot class="table-light"><tr><th colspan="2" class="text-end">Total</th><th class="text-end">Q${total.toLocaleString('es-CO')}</th></tr></tfoot>
        </table>
      </div>`;
  }
